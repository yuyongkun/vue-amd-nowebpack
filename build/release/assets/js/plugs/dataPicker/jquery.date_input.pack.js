DateInput = (function ($) {
    ...
    DateInput.prototype = {
        build: function () {
            var monthNav = $('<p class="month_nav">' + '<button class="button prev" title="[Page-Up]">&#171;</button>' + ' <span class="month_name"></span> ' + '<button class="button next" title="[Page-Down]">&#187;</button>' + '</p>');
            ...
            var yearNav = $('<p class="year_nav">' + '<button class="button prev" title="[Ctrl+Page-Up]">&#171;</button>' + ' <span class="year_name"></span> ' + '<button class="button next" title="[Ctrl+Page-Down]">&#187;</button>' + '</p>');
            ...
            this.dateSelector = this.rootLayers = $('<div class="date_selector" id="date_selector"></div>').append(nav, tableShell).insertAfter(this.input);
            //IE6-8
//            if ($.browser.msie && $.browser.version < 7) {//jquery1.9以后不再支持

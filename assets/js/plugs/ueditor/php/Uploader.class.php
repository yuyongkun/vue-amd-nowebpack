...
class Uploader
{
    ...
    private function upFile()
    {
        ...
        if (!(move_uploaded_file($file["tmp_name"], $this->filePath) && file_exists($this->filePath))) { //移动失败
            ...
            $this->imageWaterMark($this->filePath,9,'logo.png','skynet');
        }
    }
    ...
    */
    private function imageWaterMark($groundImage,$waterPos=0,$waterImage="",$waterText="",$textFont=5,$textColor="#FF0000")
    {
        ...
        {
            $temp = imagettfbbox(ceil($textFont*5),0,"STCAIYUN.TTF",$waterText);//取得使用 TrueType 字体的文本的范围
            $w = $temp[2] - $temp[6];
            $h = $temp[3] - $temp[7];
            unset($temp);
            $label = "文字区域";
        ...
        // {
            ...
            imagestring ( $ground_im, $textFont, $posX, $posY-20, $waterText, imagecolorallocate($ground_im, $R, $G, $B)); 
        // }
        //生成水印后的图片
        @unlink($groundImage);
        switch($ground_info[2])//取得背景图片的格式
